...
const CrudBasic = () => {
 ...
 className="d-flex justify-content-around">
                <button hidden={!submit} onClick={()=> { setSubmit(false)}}     his  className="btn btn-info" > Cancel</button> <button
                  hidden={!submit}
            ...
            data.list_data.map((item) => (
              <div key={item._id} className="col-md-4 col-sm-6 my-3 col-xs-12">
                <div className="card">
                  ...
                  onClick={() => { setSubmit(true); setName(item.name);setemail(item.email); setId(item._id)
                  }}
                >
                  Edit
                </button><button
